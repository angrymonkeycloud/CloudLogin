...
public interface ICloudLoginAccountStore
{
    ...
    Task<CloudSubscription?> GetSubscriptionAsync(Guid subscriptionId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CloudSubscription>> GetSubscriptionsAsync(Guid? userId, Guid? workspaceId, CancellationToken cancellationToken = default);
    Task<IReadOnlyList<CloudSubscription>> GetAllSubscriptionsAsync(CancellationToken cancellationToken = default);
    Task SaveSubscriptionAsync(CloudSubscription subscription, CancellationToken cancellationToken = default);
    Task<CloudBillingProfile?> GetBillingProfileAsync(Guid? userId, Guid? workspaceId, CancellationToken cancellationToken = default);
    Task SaveBillingProfileAsync(CloudBillingProfile profile, CancellationToken cancellationToken = default);
    ...
    Task DeleteBillingProfileAsync(Guid? userId, Guid? workspaceId, CancellationToken cancellationToken = default)
        => throw new NotSupportedException($"'{GetType().Name}' does not implement deletion.");
}
